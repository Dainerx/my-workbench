    ...
    provideReferenceLinks(text, document, base) {
        ...
        for (const match of matchAll(this.referenceLinkPattern, text)) {
            ...
            try {
                ...
                if (link) {
                    results.push(new vscode.DocumentLink(new vscode.Range(linkStart, linkEnd), vscode.Uri.parse(`command:_asciidoc.moveCursorToPosition?${encodeURIComponent(JSON.stringify([link.linkRange.start.line, link.linkRange.start.character]))}`)));
                }
            }
            catch (e) {
                // noop
